...
import { parseUpdateProductBody }                         from "@/lib/validations/product";
import { getProductBySlug, updateProduct,
         deleteProduct }                                  from "@/lib/services/product.service";
...
export async function PATCH(
  ...
  try {
    const product = await updateProduct(slug, parsed.value);
    return ok(product);
  } catch (error) {
    return handleError(error);
  }
